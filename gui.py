...
root.title(u"AutoColorIntensity")
...
def get_value(event):
    ...
    fulltime = AutoColorIntensity.main(scale, ext, name, HSV_u,
                                       HSV_l, cl, gauk, gaun, closing_on)
    tkm.showinfo('info', '>>> complete {0:.2f} sec <<<'.format(fulltime))
    return


S_title = tk.Label(text=u'GUI Mode of AutoColorIntensity')
